...
import { createDocument } from "@/app/admin/documentos/actions"
      ...
      <div className="mx-auto flex w-full max-w-[640px] flex-col gap-4 px-3.5 pb-5 pt-6 min-[390px]:px-4 sm:px-6 lg:max-w-[720px]">
        ...
        <section className="relative flex flex-col items-center text-center">
          ...
          <h1 className="relative z-10 mt-2 font-display text-[2rem] font-extrabold leading-none tracking-normal text-balance min-[390px]:text-[2.25rem] sm:text-5xl">
            <span className="text-primary">{c.adminLists.createDocumentTitleStart}</span>{" "}
            <span className="text-accent">{c.adminLists.createDocumentTitleAccent}</span>
          </h1>
        </section>

        <DocumentForm action={createDocument} submitLabel={c.adminLists.createDocumentSubmit} labels={documentFormLabels(c)} />
      </div>
    </main>
  )
}
