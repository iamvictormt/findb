...
export default async function InfluencerDashboardPage({ params }: PageProps) {
  ...
  const [campaigns, assets, metrics] = await Promise.all([
  ...
  const referralUrl = makeReferralUrlFromHeaders(profile.referralSlug, await headers())
    ...
    <main className="findb-shell relative min-h-screen overflow-hidden text-foreground">
      ...
      <div className="mx-auto flex w-full max-w-[640px] flex-col gap-4 px-3.5 pb-5 pt-6 min-[390px]:px-4 sm:px-6 lg:max-w-[720px]">
        <AuthenticatedTopBar
          left={
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-full bg-white/72 px-3 py-2 text-xs font-extrabold text-primary shadow-sm ring-1 ring-white/80 backdrop-blur transition hover:bg-white hover:text-accent"
            >
          ...
          right={
            <span className="rounded-full bg-white/72 px-3 py-2 text-xs font-extrabold text-primary shadow-sm ring-1 ring-white/80 backdrop-blur">
              {c.common.status}: {statusLabel(profile.status, c.common)}
            </span>
          }
        />
        ...
        <section className="rounded-[1.15rem] bg-white/88 p-3 shadow-[0_10px_24px_-18px_rgba(33,33,156,0.45)] ring-1 ring-white/80 backdrop-blur min-[390px]:rounded-[1.2rem] min-[390px]:p-4 sm:p-5">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <h2 className="font-display text-lg font-extrabold leading-tight text-primary">
                {c.account.shareUrl}
              </h2>
            </div>
          </div>
          ...
          <div className="mt-3 grid gap-3 min-[420px]:grid-cols-[116px_1fr]">
            <div
              className="rounded-[0.9rem] bg-white p-2 shadow-[0_12px_30px_-24px_rgba(33,33,156,0.72)] ring-1 ring-primary/8"
            ...
            <div className="grid content-center gap-2">
              ...
              <p className="rounded-[0.8rem] bg-accent/8 px-3 py-2 text-[11px] font-bold leading-relaxed text-primary/82">
                {c.account.shareHint}
              </p>
            </div>
          </div>
        ...
        <section className="grid gap-2.5 rounded-[1.15rem] bg-white/88 p-3 shadow-[0_10px_24px_-18px_rgba(33,33,156,0.45)] ring-1 ring-white/80 backdrop-blur min-[390px]:rounded-[1.2rem] min-[390px]:p-4 sm:grid-cols-3 sm:p-5">
          <MiniStat icon={BadgeEuro} label={c.common.available} value={formatEuro(profile.availableCents)} />
          <MiniStat icon={BadgeEuro} label={c.common.pending} value={formatEuro(profile.pendingCents)} />
          <MiniStat icon={Trophy} label={c.common.code} value={profile.referralCode} />
        </section>
            ...
            {campaigns.map((campaign) => (
              <div key={campaign.id} className="rounded-lg bg-primary/5 p-3 ring-1 ring-primary/6">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-white px-2.5 py-1 text-[11px] font-extrabold text-accent">
        ...
        <section className="grid gap-3">
          <Panel title={c.account.statement}>
            {profile.earnings.length ? (
              ...
              <p className="rounded-lg bg-primary/5 p-3 text-xs font-semibold leading-relaxed text-muted-foreground">
                {c.account.noEarnings}
              </p>
            )}
          </Panel>

          <Panel title={c.account.rankingChallenges}>
            {[c.account.weeklyRanking, c.account.monthlyRanking, c.account.specialMissions, c.account.referralBonus].map((item) => (
